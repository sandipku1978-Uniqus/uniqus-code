...
export const SPAWN_AGENTS_TOOL: Anthropic.Tool = {
  name: "spawn_agents",
  description:
    "Delegate focused work to one or more specialized sub-agents that run autonomously in THIS sandbox and report back. Pass MULTIPLE entries to run them IN PARALLEL — use this to parallelize INDEPENDENT work (e.g. audit the API while a design agent restyles the landing page). Each sub-agent has the same tools you do (read/write files, run commands, preview, connectors) EXCEPT it cannot spawn further sub-agents. Use this to parallelize independent work or get a focused expert pass; do NOT use it for trivial steps you can just do yourself, and do NOT spawn agents whose tasks edit the SAME files at the same time (they would clobber each other). Each sub-agent's final report is returned to you. Types: " +
