# Gate 15 — working notes for Codex
...
- **Model comparisons: source them from Artificial Analysis**
  (`artificialanalysis.ai`), not from the vendor's launch post. Vendors publish
  their own harness + their own competitor numbers; AA re-runs everything on one
  harness.
  - **Never cite SWE-Bench Pro or SWE-Bench Multilingual.** Both are saturated —
    frontier models cluster tightly enough that the ranking is noise, so they
    can't discriminate. (AA's own Coding Agent Index excludes them.)
  - Prefer **DeepSWE**, **CursorBench**, **Terminal-Bench v2**, and AA's
    composite indices: the **Intelligence Index** and the **Coding Agent Index**
    (= DeepSWE + Terminal-Bench v2 + SWE-Atlas QnA).
  - **Always quote cost per task next to the score.** Headline $/Mtok badly
    understates token-efficient models, and $/task is the number that maps to
...
- **Z.ai (GLM) runs on the OpenAI-style Chat Completions endpoint**, not GLM's
  ...
  (`"high"`/`"max"`, mapped from our **low/medium/high→"high", xhigh/max→"max"**)
  plus `thinking:{type:"enabled"}`; the reasoning trace streams as
  `delta.reasoning_content`. **Do NOT map medium OR high→"max":** GLM-5.2 accepts
  seven rungs but collapses them to three real tiers (none/minimal→no thinking,
  low/medium/high→high, xhigh/max→max), so `"high"` is its own middle tier. Mapping
  medium to "max" silently ran the deepest tier and let GLM spiral in thinking for
  minutes; mapping *high* to "max" (fixed 2026-07-09) did the same one rung up AND
  made the composer's two GLM rungs — `thinkingEffortsForModel` returns exactly
  `["high","max"]` — send an identical payload, so the picker did nothing. Only
  xhigh/max may opt into "max". GLM also needs
...
- **Built-in web search: all four providers.** Anthropic server-side
  ...
  Gemini).
- **Thinking effort** (`ThinkingEffort` = low/medium/high/xhigh/max in
  `api-types`; `thinkingEffortsForModel` is the source of truth for which rungs a
  given model actually accepts — GLM shows only high/max, Google caps at high): a
  ...
  - **Z.ai (GLM)** → top-level `reasoning_effort` (low/medium/high→`"high"`,
    xhigh/max→`"max"`; neither medium nor high may be "max") +
