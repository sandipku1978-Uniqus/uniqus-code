# Running your Gate 15 app on your own infrastructure

This is the how-to for a customer who wants to run the app Gate 15 built on
**their own** infrastructure — their CI/CD, their container registry, their
Kubernetes or VMs — instead of Gate 15's built-in public-cloud deploy button
...
runtime dependency on Gate 15 services, so you can package it however your
...
1. Connect GitHub in Gate 15 and use **Publish to GitHub**. Gate 15 creates a
...
You own the repo, the secrets, and the pipeline. Gate 15 is out of the loop once
the code is pushed.
...
> endpoint. If your Gate 15 deployment doesn't expose it yet, use **Path 1**
> (GitHub publish) to obtain the same source — the code is identical.

### 2b. Add a Dockerfile (+ .dockerignore)

Gate 15 does **not** currently auto-generate a `Dockerfile`, `.dockerignore`, or
...
Dockerfile for that stack the way you would for any ordinary app, e.g.:
...
stacks, use those — there's nothing Gate 15-specific to preserve or strip out.
...
out (Gate 15's export path excludes them before the code ever leaves the
...
Gate 15 has a built-in one-click **Deploy to Vercel**
([`services/orchestrator/src/deploy.ts`](../services/orchestrator/src/deploy.ts)),
...
Only the **app Gate 15 generates** is portable. **Self-hosting or air-gapping the
Gate 15 control plane itself** — the orchestrator, the Firecracker microVM
...
code in a sandbox) still runs on Gate 15-hosted infrastructure.
...
Gate 15 services — that's why Paths 1 and 2 work cleanly (Path 2 just means
writing an ordinary Dockerfile for the stack, same as for any other app).
